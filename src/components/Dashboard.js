  ...
  const getFilteredData = () => {
    ...
    if (activeCrossover === "all") {
      ...
      functionNames.forEach((functionName) => {
        ...
        if (showSTS) {
          ["exponential", "binomial", "onepoint", "twopoint"].forEach(
            (crossover) => {
              const stsData = getFunctionDataByCrossoverAndSelection(
                crossover,
                "sts",
                functionName
              );
        ...
        if (showGreedy) {
          ["exponential", "binomial", "onepoint", "twopoint"].forEach(
            (crossover) => {
              const greedyData = getFunctionDataByCrossoverAndSelection(
                crossover,
                "greedy",
                functionName
              );
      ...
      <header className="App-header">
        <div className="header-content">
          <div className="header-text">
            ...
            <p>
              Comparison of DE Models across Various Fitness Functions,
              Crossover Methods, and Selection Strategies
            </p>
          </div>
          ...
          <div className="charts-grid">
            {Object.entries(filteredData).map(
              ([functionName, functionData]) => (
                <div
                  key={`${functionName}-${activeCrossover}-${showSTS}-${showGreedy}-${chartType}`}
